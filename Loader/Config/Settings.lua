...
settings.DataStoreKey = "CHANGE THIS TO SOMETHING RANDOM"			 -- CHANGE THIS TO SOMETHING RANDOM! Key used to encrypt all datastore entries; Changing this will lose any saved data
...
settings.DefaultTheme = "BasicAdmin" -- Theme to be used as a replacement for "Default". The new replacement theme can still use "Default" as its Base_Theme however any other theme which references "Default" as its redirects to this theme.
...
settings.Ranks = {
	...
	["Admins"] = {
		Level = 200;
		Users = {
			"username here","username here","username here","username here","username here"	--// Add users here
		};
	};
	...
	["HeadAdmins"] = {
		Level = 300;
		Users = {
			"username here","username here","username here"--// Owner's most trusted people should be the only people who have headadmin be careful!
		};
	};
	...
	["Creators"] = {
		...
		Users = {
			"username here" --// Add users here (Also, don't forget quotations and all that)
		};
	};
};
...
settings.ConsoleKeyCode = "Semicolon"	-- Keybind to open the console; Rebindable per player in userpanel; KeyCodes: https://developer.roblox.com/en-us/api-reference/enum/KeyCode
settings.WebPanel_Enabled = false;
wait(1)
...
settings.G_Access_Key = "c29755d9773f93a5e4960afe2151c4332f0f0125ef5e9e86e3897369709fe553"	-- Key required to use the _G access API; Example_Key will not work for obvious reasons
settings.G_Access_Perms = "Write" 		-- Access perms
settings.Allowed_API_Calls = {
	Client = true;				-- Allow access to the Client (not recommended)
	Settings = true;			-- Allow access to settings (not recommended)
	DataStore = true;			-- Allow access to the DataStore (not recommended)
	Core = true;				-- Allow access to the script's core table (REALLY not recommended)
	Service = true;			-- Allow access to the script's service metatable
	Remote = true;				-- Communication table
...
settings.BanMessage = "blacklisted by kronos moderation system: Exploit Detected or/ admin initiated ban || If this was a error contact the owner of the game or staff"				-- Message shown to banned users upon kick
settings.LockMessage = "Kronos Moderation System Server Lock Active: Not Whitelisted"	-- Message shown to people when they are kicked while the game is :slocked
settings.SystemTitle = "Kronos Moderation System Alert"		-- Title to display in :sm and :bc
...
settings.HelpButtonImage = "http://www.roblox.com/asset/?id=6109242087" -- Change this to change the help button's image
